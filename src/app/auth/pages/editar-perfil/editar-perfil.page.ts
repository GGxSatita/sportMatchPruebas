...
export class EditarPerfilPage implements OnInit {
  ...
  imageUrl: string = ''; // URL actual de la foto de perfil
  profileImage: SafeResourceUrl | null = null; // Imagen seleccionada temporalmente
  selectedFile: File | null = null; // Archivo temporal
  ...
  getDatosProfile(uid: string) {
    this.firestoreService
      .getDocumentChanges<ModelsAuth.UserProfile>(`${ModelsAuth.PathUsers}/${uid}`)
      .subscribe((res) => {
        if (res) {
  ...
  private async actualizarDatosUsuario(data: { displayName?: string; photoURL?: string }) {
    try {
      await this.autenticacionService.updateProfile({
        ...
        photoURL: data.photoURL || this.user.photoURL,
      });

      await this.autenticacionService.reloadUser();
      const updatedUser = this.autenticacionService.getCurrentUser();

      const updateData = {
        name: updatedUser?.displayName,
        photo: data.photoURL || updatedUser?.photoURL,
        edad: this.profileForm.value.nuevaEdad,
        deporteFavorito: this.profileForm.value.deporteFavorito,
      };

      await this.firestoreService.updateDocument(
        `${ModelsAuth.PathUsers}/${updatedUser?.uid}`,
        updateData
      );
      ...
      this.user = {
        ...
        name: updatedUser?.displayName,
        photo: data.photoURL || 'assets/img/default-profile.png',
      };
    } catch (error) {
